...
public class Whisk {
    ...
    func whiskAPI(accessKey: String, accessToken: String, namespace: String, verb: String, type: WhiskType, package: String?, name: String, parameters: AnyObject?, isSync: Bool, callback: (reply: Dictionary<String,AnyObject>?, error:WhiskError?) ->Void) throws {
        ...
        let task: URLSessionDataTask = sess.dataTask(with: request) {
                                            ...
                                            if let whiskResponse = reply["response"] as? [String:AnyObject] {
                                                ...
                                                if let actionResult = whiskResponse["result"] {
                                                    ...
                                                    if let payloadString = payload as? String {
                                                        do {
                                                            let payloadObj:AnyObject? = try JSONSerialization.jsonObject(with: payloadString.data(using: String.Encoding.utf8)!, options:[])
